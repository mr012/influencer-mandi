...
import {createIntro} from './introFilm.js';
...
let timer;const startFlip=()=>{clearTimeout(timer);clearInterval(interval);timer=setTimeout(()=>{flip();interval=setInterval(flip,5000)},1000)};
const replay=document.createElement('button');replay.type='button';replay.className='lp-replay';replay.setAttribute('aria-label','Replay intro');replay.innerHTML='<svg aria-hidden="true" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.6"><path d="M2.5 8a5.5 5.5 0 1 0 1.7-4M2.5 2.5v3h3"/></svg><span>Replay intro</span>';page.prepend(replay);
const seenKey='mandi:intro:seen';
let seen=false;try{seen=localStorage.getItem(seenKey)==='1'}catch{}
const film=createIntro(el,{onStart(){clearTimeout(timer);clearInterval(interval);angle=360;const logo=el.querySelector('.lp-flip');logo.style.transition='none';logo.style.transform='perspective(4em) rotateX(360deg)';void logo.offsetWidth;logo.style.transition=''},onEnd(){try{localStorage.setItem(seenKey,'1')}catch{}startFlip();replay.focus({preventScroll:true})}});
const replayIntro=()=>{if(film.state!=='playing'&&film.state!=='skipping')film.play()};
replay.addEventListener('click',replayIntro);
// Defer until mounting and route setup have attached the screen to the document.
const introFrame=requestAnimationFrame(()=>{if(!seen&&!reduced.matches)film.play();else if(document.readyState==='complete')startFlip();else window.addEventListener('load',startFlip,{once:true})});
...
return ()=>{cancelAnimationFrame(introFrame);film.destroy();replay.removeEventListener('click',replayIntro);replay.remove();tiltCleanups.forEach(cleanup=>cleanup());window.removeEventListener('load',startFlip);clearTimeout(timer);clearInterval(interval);observer.disconnect();page.removeEventListener('pointermove',move);page.removeEventListener('pointerleave',leave)};
